<?php
	...
	<script>
		$j(function() {
			...
			Cal._fullCal = new FullCalendar.Calendar($j('#' + calId).get(0), {
				...
				eventSources: [
					{
						url: 'calendar-events-expired-batch.json.php',
						failure: function() {
							$j('#' + calId + '-events-loading-error').removeClass('hidden');
						}
					},
					{
						url: 'calendar-events-incoming-transaction.json.php',
						failure: function() {
							$j('#' + calId + '-events-loading-error').removeClass('hidden');
						}
					},
				...
				Cal.newEventButtons.create([
					{
						type: 'expired-batch',
						color: 'danger',
						title: AppGini.Calendar.Translate.word('new_x', { event: 'expired batch' }),
						table: 'batches'
					},
					{
						type: 'incoming-transaction',
						color: 'info',
						title: AppGini.Calendar.Translate.word('new_x', { event: 'incoming transaction' }),
						table: 'transactions'
					},
				]);
			});
	...
	<div class="page-header"><h1>
		...
		Incoming and expiring calendar				</h1></div>

	<div 
		id="incoming-expired-loading" 
	...
	<div id="incoming-expired-events-loading-error" class="hidden alert alert-warning text-center">
