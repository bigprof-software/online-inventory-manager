...
<?php }else{ ?>
	...
	<button class="btn btn-lg btn-default pull-right search_mode" type="submit" name="Filter_x" value="1">Switch to advanced search mode</button>
			...
			<div class="page-header"><h1>
				<a href="suppliers_view.php" style="text-decoration: none; color: inherit;">
					<img src="resources/table_icons/administrator.png"> 					Suppliers Filters</a>
			</h1></div>
	...
	<div class="row vspacer-lg" style="border-bottom: dotted 2px #DDD;" >
		...
		<div class="hidden-xs hidden-sm col-md-3 vspacer-lg text-right"><label for="">Contact person</label></div>
		<div class="hidden-md hidden-lg col-xs-12 vspacer-lg"><label for="">Contact person</label></div>
		...
		<div class="col-xs-3 col-xs-offset-9 col-md-offset-0 col-md-1">
			...
			<?php
				...
				if ($FilterValue[4]) {
					...
					$text = htmlspecialchars($FilterValue[4]);
		...
		<div class="col-md-8 col-lg-6 vspacer-md">	
			<div id="6_DropDown"><span></span></div>
		</div>
		<input type="hidden" class="populatedOptionsData" name="4" value="<?php echo htmlspecialchars($FilterValue[4]); ?>" >
	...
	<script>
		var populate_6 = <?php echo $filtervalueObj ;?>		
		$j(function () {
			$j("#6_DropDown").select2({
				data: <?php echo $optionsList; ?>}).on('change', function (e) {
				$j("#6_currValue").val(e.added.text);
			});


			/* preserve the applied filter and show it when re-opening the filters page */
			if ($j("#6_currValue").val().length) {
				$j("#6_DropDown").select2('data', populate_6 );
			}
		});
	</script>
